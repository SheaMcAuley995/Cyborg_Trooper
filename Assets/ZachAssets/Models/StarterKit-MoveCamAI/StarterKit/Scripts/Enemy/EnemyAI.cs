...
public class EnemyAI : MonoBehaviour {
	...
	void EnemyMove()
	{
		...
		{
			if(Vector2.Distance(transform.position,player.position) > 2) // If the distance from the player is greater than a number then continue
			{
				EnemyRotate(player.position); // Calls the rotate function sending the players position
				rb.MovePosition(Vector2.MoveTowards(transform.position, player.position, Time.deltaTime * speed)); // Move the enemy towards the players position
		...
		else if(FindLastPosition() != null) // If the player has NOT been spotted and a last position has been found then continue **FindLastPosition() returns a GameObject
		{
			...
			rb.MovePosition(Vector2.MoveTowards(transform.position, FindLastPosition().transform.position, Time.deltaTime * speed)); // Move the enemy towards the found last position game object
			anim._animRun = true; // Enable the run animation
	...
	private void EnemyRotate(Vector2 position)
	{
	...
	private void EnemyAvoid()
	{
		...
		if (Physics.Raycast(transform.position,-transform.right,out left,avoidDist, ~(lastPosLayer | enemyLayer | playerLayer)))
		{
			...
			rb.AddForce(transform.right * avoidSpeed); 	// Add force to the right
		} 
		else // If the cast no is no longer hitting anything the continue
		{
			rb.velocity = Vector2.zero; // Stops all rigidbody movement, this removes any sliding around
		}
		...
		if (Physics.Raycast(transform.position,transform.right,out right,avoidDist, ~(lastPosLayer | enemyLayer | playerLayer)))
		{
			...
			rb.AddForce(-transform.right * avoidSpeed); // Add force to the left
		} 
		else // If the cast no is no longer hitting anything the continue
		{
			rb.velocity = Vector2.zero;// Stops all rigidbody movement, this removes any sliding around
